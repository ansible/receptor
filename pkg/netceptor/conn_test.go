...
func TestAcceptLoopContextCancelCleanup(t *testing.T) {
	t.Run("defer ccancel called when RemoteAddr is not Receptor Addr", func(t *testing.T) {
		// This tests the edge case at conn.go:310-320 where RemoteAddr
		// type assertion fails (ok=false). The defer ccancel() at line 315-320
		// ensures the context cancel function is called, preventing a leak.
		//
		// Uses TestOnContextCancelDeferCalled hook to verify defer is executed.
		...
		// Reset and track if defer ccancel() was called
		netceptor.TestOnContextCancelDeferCalled.Store(false)
		defer func() {
			netceptor.TestOnContextCancelDeferCalled.Store(false)
		}()
		...
		// Setup successful connection with non-Receptor RemoteAddr (triggers ok=false)
		...
		// Verify defer ccancel() was called
		if !netceptor.TestOnContextCancelDeferCalled.Load() {
			t.Error("Expected defer ccancel() to be called when ok=false, but it was not")
		}
	})
}

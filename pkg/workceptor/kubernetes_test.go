...
func TestKubeUnit_LoggingMethodSelection(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			original := os.Getenv("RECEPTOR_KUBE_SUPPORT_RECONNECT")
			defer func() {
				if original != "" {
					os.Setenv("RECEPTOR_KUBE_SUPPORT_RECONNECT", original)
				} else {
			...
			// This is the same logic used in runWorkUsingLogger to determine which method to call
			var useReconnect bool
			switch os.Getenv("RECEPTOR_KUBE_SUPPORT_RECONNECT") {
			case "enabled", "auto":
				useReconnect = true
			default:
				// "disabled", empty, or any invalid value
				useReconnect = false
			}
			...
			if useReconnect {
				...
				t.Logf("Would use kubeLoggingNoReconnect (target function)")
			}
		})
	}
}

...
func TestQuicConnectTimeout(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Error listening on TCP: %s", err)
	}
	err = n1.AddBackend(b1)
	if err != nil {
		t.Fatalf("Error starting backend: %s", err)
	}
	...
	if err != nil {
		t.Fatalf("Error dialing on TCP: %s", err)
	}
	...
	if err != nil {
		t.Fatalf("Error listening on Receptor network: %s", err)
	}
	go func() {
			...
			for !done {
				...
				if err == io.EOF {
					...
					if strings.Contains(err.Error(), "no recent network activity") {
						...
						t.Fatalf("Read error in Receptor listener: %s", err)
					}
				...
				if n > 0 {
					...
					if err != nil {
						t.Fatalf("Write error in Receptor listener: %s", err)
